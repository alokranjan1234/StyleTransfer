...
ax1.imshow(imgcnvt(content),label = "Content")
ax2.imshow(imgcnvt(style),label = "Style")
plt.show()
...
for i in range(1,epochs+1):
    ...
    style_loss = 0
    for layer in style_wt_means:
        style_gram = style_grams[layer]
        target_gram = target_features[layer]
        _,d,w,h = target_gram.shape
        target_gram = gram_matrix(target_gram)
        style_loss += (style_wt_means[layer]*torch.mean((target_gram-style_gram)**2))/(d*w*h)
         
    total_loss = content_wt*content_loss + style_wt*style_loss

    if i%10 == 0:
        print("total loss ",i," ", total_loss)
    optimizer.zero_grad()
